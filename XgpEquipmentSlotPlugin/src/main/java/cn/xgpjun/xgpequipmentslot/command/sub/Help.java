...
public class Help implements TabExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        if(sender.isOp()){
            ...
            sender.sendMessage(Message.apiAttribute.replace("{name}", AttributeManager.getAttributeAPI().getName()).replace("{author}",AttributeManager.getAttributeAPI().getAuthor()).replace("{version}",AttributeManager.getAttributeAPI().getVersion()));
