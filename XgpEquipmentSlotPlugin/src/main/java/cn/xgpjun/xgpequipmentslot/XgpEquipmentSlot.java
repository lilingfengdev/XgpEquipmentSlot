...
import cn.xgpjun.xgpequipmentslot.apiWrapper.APIWrapperLoader;
import cn.xgpjun.xgpequipmentslot.apiWrapper.XESExpansionLoader;
...
public final class XgpEquipmentSlot extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        if (!new File(getDataFolder(), "AttributeAPI").exists()) {
            new File(getDataFolder(), "AttributeAPI").mkdirs();
        }

        if (!new File(getDataFolder(), "XESExpansion").exists()) {
            new File(getDataFolder(), "XESExpansion").mkdirs();
        }
        log("§c§m====§e§m====§a§m====");
        log("author: xgpjun");
        log("QQ:357004885");
        ...
        APIWrapperLoader.loadAttributeAPI();
        XESExpansionLoader.load();
    }
    ...
    public void onDisable() {
        ...
        DataManager.temp.values().forEach(stringPlayerSlotInfoMap -> {
            stringPlayerSlotInfoMap.values().forEach(DataManager::saveToDatabase);
        });
        DataManager.close();
        Potions.cancel();
